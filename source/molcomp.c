...
bool VER = false;
char ALG[100];
char DIST[100]; // print average dist & max dist?
...
for (i=3;i<argc;i++) {
  // populate DIST
  // default is false
  if (  strncmp(strlwr(argv[i]), "--dist=", 7) == 0) {
    char *ptr;
    ptr = strtok(strlwr(argv[i]), "=");
    // take second argument
    ptr = strtok(NULL, "=");
    if ( strcmp (ptr, "false") == 0 ) {
      ...
      printf ("Your --dist=%s is not clear\n", ptr);
      exit(0);
    }
  break;
  }
...
// check that eveything is stored in coord1 and coord2
...
if (strcmp(ALG, "std")==0) {
  ...
  if (strcmp(DIST, "false") == 0) {
    ...
    KABSH (coord1, coord2, &coord1mod, NAT1, &RMSDST);
    if (VER) {
    ...
    KABSHDIST (coord1, coord2, &coord1mod, NAT1, &RMSDST, &DAVST, &DMAXST);
    if (VER) {
